...
out to the network when you ask it to (Open from url…). This embedded
sample book doubles as the manual — drop in
...
The **Strip remote media** switch in the document menu decides how
media in an imported file is handled. **Off** (the default) keeps remote
media exactly as the author wrote it: every `https://` image, video and
audio url renders from its source and survives into every export.
**On** runs a privacy pass over the source before it is rendered or
stored: image, video and audio constructs are removed, so the document
reads — and exports — as pure local text. Links, text and code are
untouched, and Markdown examples inside code fences or code spans are
structurally protected. Either way the decision is made at import time
...
- **Export publishable HTML** — an exact replica of this entire reader with
  ...
  "Strip remote media" and "Include document menu in publication"
