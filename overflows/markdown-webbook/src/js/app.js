 ...
 *   1.8.7 adds the composited zoom/pan engine),
...
(function () {
    ...
    /* ==================== Image lightbox (M5, Proposal A; zoom 1.8.7) ====
       Click any content image to read it at full size. Esc, a click on
       the backdrop and the close button dismiss it; clicks on the image,
       its caption or the letterbox surface never do — that surface is
       the zoom/pan playground. Images inside links (badges) never zoom —
       ...
       Zoom/pan engine (1.8.7, zero-dependency): every gesture mutates
       exactly two numbers — scale (1x..5x) and the (tx, ty) offset — and
       one composited declaration, `transform: translate3d(..) scale(..)`,
       is written once per animation frame, so zooming and panning run on
       the compositor and never trigger layout or paint.

         - Mobile pinch: PointerEvents track both contacts; the distance
           ratio maps to scale and the midpoint delta pans, with the
           anchor following the midpoint. `touch-action: none` on the
           modal keeps the browser's own zoom/scroll/rubber-band out of
           the viewport while the lightbox is open.
         - Mouse wheel: damped exponential steps (one notch ≈ x1.22),
           anchored at the cursor's exact position — never the viewport
           centre.
         - Trackpad pinch: browsers encode it as a wheel event with
           ctrlKey === true and small high-frequency deltas; it is
           normalised with a stronger exponential damping curve so a
           physical pinch feels continuous instead of stepped.
         - Boundaries: scale clamps to [1x, 5x]; pan is clamped so at
           least a 40px band of the scaled image always stays inside the
           viewport; landing back on 1x resets the matrix to the default
           centered transform (animated, via the is-settling class). */
    ...
    var Lightbox = {
        ...
        MIN_SCALE: 1,
        MAX_SCALE: 5,
        /* Pan clamp: minimum visible band (px) of the scaled image. */
        PAN_MARGIN: 40,

        scale: 1,
        tx: 0,
        ty: 0,
        /* base — geometry of the UNtransformed image element. The flex
           layout fixes the element box for the whole overlay session, so
           one snapshot (captured on open, image load and resize) serves
           every gesture; no gesture math ever reads layout. */
        base: { cx: 0, cy: 0, w: 0, h: 0 },

        pointers: new Map(),
        gesture: null, /* {mode:"pan",px,py} | {mode:"pinch",d,mx,my} */
        gestureMoved: false,
        suppressClick: false,
        framePending: false,
        settleTimer: null,
        ...
        init: function () {
            ...
            var el = Lightbox.el;
            /* Dismissal surface: ONLY the backdrop around the figure (the
               safe-area band outside it) and the close button. Clicks on
               the figure — image, caption, letterbox — bubble with those
               elements as target and are ignored here. A drag that ends
               outside the image fires its trailing click on the common
               ancestor of the down/up targets — this element — and would
               dismiss mid-gesture, so gesture-ending clicks are
               swallowed once (suppressClick). */
            ...
            /* passive: false — the modal must be able to claim every
               wheel gesture (page scroll, browser pinch-zoom). */
            el.addEventListener(
                "wheel",
                function (e) {
                    Lightbox.onWheel(e);
            ...
            /* The zoom surface is gesture-only; native image dragging
               (ghost image while panning) would fight the pan. */
            el.addEventListener("dragstart", function (e) {
                e.preventDefault();
            });
            Lightbox.img.addEventListener("load", function () {
                if (!Lightbox.open) return;
                Lightbox.captureBase();
                Lightbox.clampPan();
                Lightbox.scheduleApply();
            });
        ...
        openFor: function (sourceImg) {
            ...
            /* Layout is live again once unhidden — snapshot the element
               box the gestures will work against. */
            Lightbox.captureBase();
        ...
        resetTransform: function () {
            ...
            Lightbox.gesture = null;
            Lightbox.gestureMoved = false;
            Lightbox.suppressClick = false;
            Lightbox.framePending = false;
            if (Lightbox.settleTimer) {
                clearTimeout(Lightbox.settleTimer);
                Lightbox.settleTimer = null;
            }
            Lightbox.img.style.transform = "";
            Lightbox.el.removeAttribute("data-zoom");
            Lightbox.el.classList.remove("is-panning", "is-settling");
        },

        captureBase: function () {
            var r = Lightbox.img.getBoundingClientRect();
            /* The visual rect centre equals the layout centre plus the
               current translate (scale is applied about the layout
               centre), so the untransformed geometry survives a live
               transform — this also holds on resize while zoomed. */
            Lightbox.base.cx = r.left + r.width / 2 - Lightbox.tx;
            Lightbox.base.cy = r.top + r.height / 2 - Lightbox.ty;
            Lightbox.base.w = Lightbox.scale > 0 ? r.width / Lightbox.scale : 0;
            Lightbox.base.h = Lightbox.scale > 0 ? r.height / Lightbox.scale : 0;
        },
        ...
        applyTransform: function () {
            Lightbox.img.style.transform =
                ...
                Lightbox.scale +
                ")";
            if (Lightbox.scale > 1) Lightbox.el.setAttribute("data-zoom", "in");
            else Lightbox.el.removeAttribute("data-zoom");
        },

        scheduleApply: function () {
            if (Lightbox.framePending) return;
            Lightbox.framePending = true;
            (window.requestAnimationFrame || function (fn) { setTimeout(fn, 16); })(
                function () {
                    Lightbox.framePending = false;
        ...
        /* Harness hook: run a pending frame synchronously (jsdom has no
           compositor; the smoke suite drives real frames). */
        flush: function () {
            if (!Lightbox.framePending) return;
            Lightbox.framePending = false;
            Lightbox.applyTransform();
        ...
        /* Zoom by `factor`, keeping the content point under viewport
           position (px, py) pinned where the cursor put it:
             t' = (P - C)(1 - k) + k·t     with k = s'/s
           (derived from V(p) = C + t + s·(p - C), transform-origin at
           the element centre). Never snaps to the viewport centre. */
        zoomAt: function (px, py, factor) {
            if (!Lightbox.base.w || !Lightbox.base.h) return;
            var prev = Lightbox.scale;
            var next = Math.min(
                Lightbox.MAX_SCALE,
                Math.max(Lightbox.MIN_SCALE, prev * factor),
            );
            if (next === prev) {
                /* Pinned at a bound — only the pan clamp can still move. */
                Lightbox.clampPan();
                Lightbox.scheduleApply();
                return;
            }
            var k = next / prev;
            Lightbox.tx = (px - Lightbox.base.cx) * (1 - k) + k * Lightbox.tx;
            Lightbox.ty = (py - Lightbox.base.cy) * (1 - k) + k * Lightbox.ty;
            Lightbox.scale = next;
            Lightbox.clampPan();
            if (Lightbox.scale === Lightbox.MIN_SCALE) {
                Lightbox.tx = 0;
                Lightbox.ty = 0;
                if (Lightbox.pointers.size === 0) Lightbox.settleHome();
            }
            Lightbox.scheduleApply();
        },

        /* Overflow clamp: the scaled image may be panned to any edge but
           never off the canvas — a PAN_MARGIN band always stays inside
           the viewport. */
        clampPan: function () {
            var m = Lightbox.PAN_MARGIN;
            var vw = window.innerWidth || Lightbox.el.clientWidth || 1024;
            var vh = window.innerHeight || Lightbox.el.clientHeight || 768;
            var hw = (Lightbox.base.w * Lightbox.scale) / 2;
            var hh = (Lightbox.base.h * Lightbox.scale) / 2;
            Lightbox.tx = Math.max(
                m - hw - Lightbox.base.cx,
                Math.min(vw - m + hw - Lightbox.base.cx, Lightbox.tx),
            );
            Lightbox.ty = Math.max(
                m - hh - Lightbox.base.cy,
                Math.min(vh - m + hh - Lightbox.base.cy, Lightbox.ty),
            );
        },

        settleHome: function () {
            if (Lightbox.settleTimer) clearTimeout(Lightbox.settleTimer);
            Lightbox.el.classList.add("is-settling");
            Lightbox.settleTimer = setTimeout(function () {
                Lightbox.settleTimer = null;
                Lightbox.el.classList.remove("is-settling");
            }, 240);
        },
        ...
        onWheel: function (e) {
            if (!Lightbox.open) return;
            e.preventDefault();
            Lightbox.cancelSettle();
            var dy = e.deltaY;
            if (e.deltaMode === 1) dy *= 16; /* lines -> px */
            else if (e.deltaMode === 2) dy *= 400; /* pages -> px */
            var factor;
            if (e.ctrlKey) {
                /* Trackpad pinch: small, high-frequency deltas — the
                   exponential map with strong damping keeps the curve
                   smooth instead of linearly jumping between steps. */
                if (dy > 120) dy = 120;
                else if (dy < -120) dy = -120;
                factor = Math.exp(-dy * 0.01);
            } else {
                /* Mouse wheel notches / two-finger scroll. */
                if (dy > 400) dy = 400;
            ...
            Lightbox.zoomAt(e.clientX, e.clientY, factor);
        },

        onPointerDown: function (e) {
            if (!Lightbox.open) return;
            /* Gestures live on the image surface only; backdrop, caption
               and close-button pointers fall through to the click layer
               untouched (canceling their pointerdown could suppress the
               very click that dismisses or closes). */
            if (e.target !== Lightbox.img) return;
            if (e.pointerType === "mouse" && e.button !== 0) return;
            /* No selection, no compatibility mouse events, no native
               drag — and, with touch-action: none, no browser pan/zoom. */
            e.preventDefault();
            Lightbox.cancelSettle();
            Lightbox.gestureMoved = false;
            Lightbox.pointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
            if (Lightbox.pointers.size === 1) {
                /* At 1x there is nothing to pan — the single pointer
                   waits; a second one turns the gesture into a pinch. */
                Lightbox.gesture =
                    Lightbox.scale > 1
                        ? { mode: "pan", px: e.clientX, py: e.clientY }
                        : null;
                if (Lightbox.gesture) Lightbox.el.classList.add("is-panning");
            } else if (Lightbox.pointers.size === 2) {
                var pts = Lightbox.points();
                Lightbox.gesture = {
                    mode: "pinch",
                ...
                Lightbox.el.classList.remove("is-panning");
            }
        },
        ...
        onPointerMove: function (e) {
            ...
            e.preventDefault();
            var p = Lightbox.pointers.get(e.pointerId);
            p.x = e.clientX;
            p.y = e.clientY;
            if (!Lightbox.gesture) return;
            if (Lightbox.gesture.mode === "pinch" && Lightbox.pointers.size >= 2) {
                var pts = Lightbox.points();
                var d = Math.hypot(pts[1].x - pts[0].x, pts[1].y - pts[0].y);
                var mx = (pts[0].x + pts[1].x) / 2;
                var my = (pts[0].y + pts[1].y) / 2;
                /* Two-finger pan first, then a cursor-anchored zoom on
                   the distance ratio — the composition of the same two
                   primitives the desktop gestures use. */
                Lightbox.tx += mx - Lightbox.gesture.mx;
                Lightbox.ty += my - Lightbox.gesture.my;
                Lightbox.gesture.mx = mx;
                Lightbox.gesture.my = my;
                Lightbox.gestureMoved = true;
                if (Lightbox.gesture.d > 0 && d > 0) {
                    Lightbox.zoomAt(mx, my, d / Lightbox.gesture.d);
                    Lightbox.gesture.d = d;
                } else {
                    Lightbox.clampPan();
                    Lightbox.scheduleApply();
                }
            } else if (
                Lightbox.gesture.mode === "pan" &&
                Lightbox.pointers.size === 1
            ) {
                Lightbox.tx += e.clientX - Lightbox.gesture.px;
                Lightbox.ty += e.clientY - Lightbox.gesture.py;
                Lightbox.gesture.px = e.clientX;
                Lightbox.gesture.py = e.clientY;
                Lightbox.gestureMoved = true;
                Lightbox.clampPan();
                Lightbox.scheduleApply();
            }
        ...
        onPointerUp: function (e) {
            ...
            if (Lightbox.pointers.size === 0) {
                /* A genuine drag/pinch must not end in a click: Chromium
                   fires the trailing click on the common ancestor of the
                   down/up targets — the backdrop — which would dismiss
                   the overlay mid-gesture. Swallow the next click once. */
                if (Lightbox.gestureMoved) {
                    Lightbox.gestureMoved = false;
                ...
                Lightbox.gesture = null;
                Lightbox.el.classList.remove("is-panning");
            } else if (Lightbox.pointers.size === 1) {
                /* Pinch -> pan handoff: rebase on the surviving pointer. */
                var p = Lightbox.points()[0];
                Lightbox.gesture =
                    Lightbox.scale > 1
                        ? { mode: "pan", px: p.x, py: p.y }
                        : null;
                if (Lightbox.gesture) Lightbox.el.classList.add("is-panning");
                else Lightbox.el.classList.remove("is-panning");
            }
        },

        points: function () {
            var out = [];
            Lightbox.pointers.forEach(function (p) {
                out.push(p);
            });
            return out;
        },
        ...
        state: function () {
            return {
                ...
                x: Math.round(Lightbox.tx * 100) / 100,
                y: Math.round(Lightbox.ty * 100) / 100,
            };
        },
    };
    ...
    const MediaTools = {
        ...
        maskCode: function (text) {
            ...
            const out = [];
            let fence = null; /* opening marker, e.g. "```" or "~~~" */
            for (const line of String(text).split("\n")) {
                ...
                out.push(line.replace(/(`+)([\s\S]*?)\1/g, (m) => put(m)));
            }
            return { text: out.join("\n"), stash };
        },

        unmaskCode: function (text, stash) {
            return String(text).replace(/\u0000M(\d+)\u0000/g, (m, n) => {
        ...
        /* Pure source-to-source rewrite: the input string is never
           mutated, every stage feeds the next, and any unexpected shape
           (a null here, a weird line there) degrades to "less is
           removed" because each regex simply fails to match. */
        stripMedia: function (md) {
            const masked = MediaTools.maskCode(md == null ? "" : md);
            let s = masked.text;
            ...
            s.replace(
                /^[ \t]{0,3}\[([^\]]+)\]:[ \t]*/gm,
                (m, label) => {
                    defined.add(MediaTools.normLabel(label));
                    return m;
                },
            );
            ...
            s.replace(/!\[[^\]]*\]\[([^\]]*)\]/g, (m, label) => {
                imgLabels.add(MediaTools.normLabel(label));
                return m;
            });
            s.replace(/!\[([^\]]*)\](?![\[(])/g, (m, label) => {
                const key = MediaTools.normLabel(label);
                if (defined.has(key)) imgLabels.add(key);
                return m;
            });
            ...
            s = s.replace(/!\[([^\]]*)\](?![\[(])/g, (m, label) =>
                imgLabels.has(MediaTools.normLabel(label)) ? "" : m,
            );
            ...
            s.replace(/\[[^\]]+\]\[([^\]]*)\]/g, (m, label) => {
                linkLabels.add(MediaTools.normLabel(label));
                return m;
            });
            s.replace(/(^|\s)\[([^\]]+)\](?![\[(])(?!\s*:)/g, (m, pre, label) => {
                linkLabels.add(MediaTools.normLabel(label));
                return m;
            });
            s = s.replace(
                /^[ \t]{0,3}\[([^\]]+)\]:[ \t]*<?([^\s>]*)>?(?:[ \t]+(?:"[^"]*"|'[^']*'|\([^)]*\)))?[ \t]*$/gm,
                (m, label) =>
                    linkLabels.has(MediaTools.normLabel(label)) ||
                    !imgLabels.has(MediaTools.normLabel(label))
                        ? m
                        : "",
            );
    ...
    window.MDWebbook = {
        ...
        lightbox: {
            ...
            /* Harness hooks (1.8.7): the zoom primitive and a synchronous
               frame flush, so jsdom checks can drive the gesture maths
               deterministically without an animation frame. */
            zoomAt: function (px, py, factor) {
                Lightbox.zoomAt(px, py, factor);
                return Lightbox.state();
            },
            flush: function () {
                Lightbox.flush();
