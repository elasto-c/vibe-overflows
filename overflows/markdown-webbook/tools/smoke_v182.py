"""v1.8.2 Chromium smoke: the widened embed format range (svg served generically
and avif become data uris), the Paste-from-clipboard keyboard shortcut
(Ctrl/Cmd+Shift+V with success/denied toasts, help-panel row, publication
exclusion) and the exact-capture metadata save (empty title -> Untitled on
tab/header/footer/placeholder, captured title on every surface + download
name), plus the carried v1.8.1 paste/embed-menu, v1.8.0 url-modal/loading/mdx
and v1.7.3 panel regressions."""
import json
import os
...
def main():
    with sync_playwright() as p:
        ...
        # ---- 1. Version -----------------------------------------------------
        check("ver.1.8.2", page.evaluate("window.MDWebbook.version") == "1.8.2")
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v182_loading.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v182_error.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v182_embed.png"))
        ...
        check("meta.empty.placeholder",
              ...
              json.dumps(reopen))
        page.screenshot(path=os.path.join(HERE, "smoke_v182_meta.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v182_modal.png"))
