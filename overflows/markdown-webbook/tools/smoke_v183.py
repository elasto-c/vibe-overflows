"""v1.8.3 Chromium smoke: extension-first image format identification (a .svg
url served as text/xml embeds as a data uri — the 1.8.2 stranding bug — while
an extensionless url with a generic header still falls through to the
magic-byte sniff), plus the carried v1.8.2 widened-format/shortcut/meta
tests, the v1.8.1 paste/embed-menu, v1.8.0 url-modal/loading/mdx and v1.7.3
panel regressions."""
import json
import os
...
def main():
    with sync_playwright() as p:
        ...
        # ---- 1. Version -----------------------------------------------------
        check("ver.1.8.3", page.evaluate("window.MDWebbook.version") == "1.8.3")
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v183_loading.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v183_error.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v183_embed.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v183_ext.png"))
        page.evaluate("document.getElementById('btn-docs').click()")
        page.evaluate("document.getElementById('mi-embed').click()")  # restore off
        page.keyboard.press("Escape")
        page.wait_for_timeout(120)
        ...
        check("meta.empty.placeholder",
              ...
              json.dumps(reopen))
        page.screenshot(path=os.path.join(HERE, "smoke_v183_meta.png"))
        ...
        page.screenshot(path=os.path.join(HERE, "smoke_v183_modal.png"))
